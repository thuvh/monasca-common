...
# $ ./build_image.sh origin/stable/queens

[ -z "$DOCKER_IMAGE" ] && \
    DOCKER_IMAGE=$(\grep DOCKER_IMAGE Dockerfile | cut -f2 -d"=")

# TODO(Dobrosla): handle getting stable branches without need for `origin/`.
: "${REPO_VERSION:=$1}"
[ -z "$REPO_VERSION" ] && \
...
# image.
TMP_DIR=$(mktemp -d)
git clone "$APP_REPO" "$TMP_DIR"
cd "$TMP_DIR" || exit 1
GIT_COMMIT=$(git rev-list "$REPO_VERSION" --max-count=1)
cd - || exit 1
rm -rf "$TMP_DIR"

# TODO(Dobroslaw): set label monasca-common version we will be using with app.
