...
enum AlarmSubExpressionIsDeterministicFunction
  ...
  INSTANCE {
    ...
    private final String deterministicMarker = "deterministic";
    ...
    private final int deterministicMarkerIndex = 0;
    ...
    public Boolean apply(@Nullable final AlarmExpressionParser.DeterministicContext ctx) {
      ...
      if (childCount == 1) {
        return this.handleOnlyKeyword(ctx);
      }

      // 2nd possible option is assignment operator
      // everything else is forbidden by gramma for expression
    ...
    private Boolean handleAssignmentOperator(final AlarmExpressionParser.DeterministicContext ctx) {
      ...
      return this.deterministicValues.indexOf(text) >= 0;
    }

    private Boolean handleOnlyKeyword(final AlarmExpressionParser.DeterministicContext ctx) {
      final ParseTree child = ctx.getChild(this.deterministicMarkerIndex);
      return child.getText().equalsIgnoreCase(this.deterministicMarker);
    }

    private boolean isValidValue(final String text) {
      return this.deterministicValues.indexOf(text) < 0
          && this.nonDeterministicValues.indexOf(text) < 0;
    }
