...
public class AlarmExpressionTest {
  ...
  public void shouldParseNonDeterministicExpression() {
    ...
    for (final AlarmExpression expr : expressions) {
      ...
      assertFalse(expr.isDeterministic());  // each expression is non-deterministic
      assertEquals(1, subExpressions.size());
      assertEquals(subExpressions.get(0), logErrorExpr);
    }
  }
  ...
  public void shouldBeNonDeterministicIfAllSubExpressionAreNonDeterministic() {
    final String expression1 = "count(log.error{hostname=1,component=A},deterministic=false,20) > 5";
    final String expression2 = "count(log.error{hostname=1,component=B},deterministic=false,20) > 10";
    final String expression3 = "count(log.error{hostname=1,component=C},deterministic=false,20) > 15";
  ...
  public void shouldBeDeterministicIfAtLeastOneExpressionIsDeterministic() {
    final String expression1 = "count(log.error{hostname=1,component=A},deterministic=false,20) > 5";
    final String expression2 = "count(log.error{hostname=1,component=B},deterministic=false,20) > 10";
