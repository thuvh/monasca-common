...
public final class Conversions {
  ...
  public static DateTime variantToDateTime(final Object variant, final DateTimeZone timeZone) {
    if (variant instanceof DateTime) {
      if(((DateTime) variant).getChronology().equals(ISOChronology.getInstance(timeZone))) {
        // if both share the time zone do not apply conversion
        return (DateTime) variant;
      }
    }
  ...
  public static <T extends Enum<T>> T variantToEnum(final Object variant, final Class<T> enumClazz) {
    ...
    throw new IllegalArgumentException(String.format("Variant of type \"%s\", and value \"%s\" is not a %s",
