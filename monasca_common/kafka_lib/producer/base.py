...
class Producer(object):
    ...
    Arguments:
        client (KafkaClient): instance to use for broker communications.
            If async_=True, the background thread will use client.copy(),
        ...
        sync_fail_on_error (bool, optional): whether sync producer should
            ...
            defaults to True.
        async_ (bool, optional): send message using a background thread,
            defaults to False.
        batch_send_every_n (int, optional): If async_ is True, messages are
            sent in batches of this size, defaults to 20.
        batch_send_every_t (int or float, optional): If async_ is True,
    ...
    Deprecated Arguments:
        batch_send (bool, optional): If True, messages are sent by a background
            thread in batches, defaults to False. Deprecated, use 'async_'
    """
    ...
    def __init__(self, client,
                 ...
                 async_=False,
                 batch_send=False,  # deprecated, use async_
    ...
    def stop(self, timeout=None):
        ...
        if not self.async_:
            log.warning('producer.stop() called, but producer is not async_')
            return
