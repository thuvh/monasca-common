...
def parse_group_expression(expression):
    try:
        result = RuleExpressionParser(expression).parse()
        result = result[0].get_struct('group')
        matchers = result['matchers']
        exclusions = result['exclusions']

        return matchers, exclusions
    except pyparsing.ParseException as e:
        raise QueryException("Unable to parse group rule with {}".format(e))


def parse_inhibit_expression(expression):
    try:
        result = RuleExpressionParser(expression).parse()
        result = result[0].get_struct('inhibit')
        source = result['source_match']
        target = result['target_match']
        equals = result['equal']
        exclusions = result['exclusions']

        return source, target, equals, exclusions
    except pyparsing.ParseException as e:
        raise QueryException("Unable to parse inhibit rule with {}".format(e))


def parse_silence_expression(expression):
    try:
        result = RuleExpressionParser(expression).parse()
        ...
        raise QueryException("Unable to parse silence rule with {}".format(e))
