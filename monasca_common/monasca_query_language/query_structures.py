    ...
    def get_filters(self):
        ...
        for key, value in right_filters.items():
            if key in left_filters and left_filters[key] != value:
                raise InvalidExpressionException("Duplicate keys specified ".format(key))
    ...
    def get_struct(self, _type):
        ...
        if _type == "silence":
            ...
            if any([self.source, self.group_by, self.excludes]):
                raise InvalidExpressionException("Silence rule contains unexpected elements")
        elif _type == "inhibit":
            ...
            if any([self.source, self.target]):
                raise InvalidExpressionException("Group rule contains unexpected elements")
        else:
            raise InvalidExpressionException("Unknown type for expression")
        return result
