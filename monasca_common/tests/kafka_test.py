...
from monasca_common.kafka.consumer import DEFAULT_KAFKA_FETCH_SIZE
...
class KafkaProducerTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.kafka_producer = KafkaProducer(FAKE_KAFKA_URL)
    ...
    def test_kafka_producer_init(self):
        self.mock_kafka_client.KafkaClient.assert_called_once_with(
            FAKE_KAFKA_URL)
    ...
    def test_kafka_producer_publish(self):
        ...
        self.kafka_producer.publish(topic, messages, key)
    ...
    def test_kafka_producer_publish_one_message_without_key(self, mock_time):
        ...
        self.kafka_producer.publish(topic, message)
    ...
    def test_kafka_producer_publish_exception(self, mock_logger):
        ...
        self.assertRaises(MockException, self.kafka_producer.publish,
                          topic, messages, key)
...
class KafkaConsumerTests(unittest.TestCase):
    ...
    def tearDown(self):
        self.mock_kafka_client = self.kafka_client_patcher.stop()
        self.mock_kafka_common = self.kafka_common_patcher.stop()
        self.mock_kafka_consumer = self.kafka_consumer_patcher.stop()

    def test_kafka_consumer_init(self):
        self.mock_kafka_client.KafkaClient.assert_called_once_with(
            FAKE_KAFKA_URL
        )
        self.mock_kafka_consumer.SimpleConsumer.assert_called_once_with(
            self.client, FAKE_KAFKA_CONSUMER_GROUP, FAKE_KAFKA_TOPIC,
            auto_commit=False,
            partitions=None,
            iter_timeout=5,
            fetch_size_bytes=DEFAULT_KAFKA_FETCH_SIZE,
            buffer_size=DEFAULT_KAFKA_FETCH_SIZE,
    ...
    def test_commit(self, mock_datetime):
        ...
        self.consumer.commit.assert_called_once_with(
            partitions=self.kafka_consumer._partitions)

    @mock.patch('monasca_common.kafka.consumer.KazooClient')
    @mock.patch('monasca_common.kafka.consumer.SetPartitioner')
    def test_iteration_failed_to_acquire_partition(
            self, mock_set_partitioner, mock_kazoo_client):

        mock_set_partitioner.return_value.failed = True

        try:
            for cons in self.kafka_consumer:
                pass
