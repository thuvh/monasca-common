...
class RestUtilsTest(unittest.TestCase):
    ...
    @mock.patch('io.IOBase')
    def test_should_return_false_for_empty_content(self, payload):
        raw_msg = u''
        payload.read.return_value = raw_msg
        self.assertEqual(False, ru.read_body(payload))
    ...
    def test_should_fail_read_text_for_application_json(self, payload):
        ...
        self.assertEqual('Expected object or value',
                         context.exception.message)

    def test_should_fail_invalid_content_type(self):
        content_type = 'video/3gp'
        with self.assertRaises(ru.UnsupportedContentType) as context:
            ru.read_body('', content_type)

        self.assertIn('Cannot read %s, not in' % content_type,
                      context.exception.message)
