...
import monasca_common.validation.metric_validation as validator
import unittest
...
class TestMetricValidation(unittest.TestCase):
    def test_valid_single_metric(self):
        metric = {'name': 'test_metric_name',
                  ...
                  "value": 5}
        validator.validate_metrics(metric)
    ...
    def test_valid_metric_unicode_dimension_value(self):
        ...
        validator.metric_dimensions(dimensions)
    ...
    def test_valid_metric_value(self):
        value = 23
        validator.metric_value(value)

    def test_valid_metric_timestamp(self):
        timestamp = 1405630174123
        validator.metric_timestamp(timestamp)

    def test_valid_metric_unicode_metric_name(self):
        metric_name = unichr(6021)
        validator.metric_name(metric_name)

    def test_invalid_metric_name(self):
        metric_name = "TooLarge" * 255
        self.assertRaises(validator.InvalidMetricName,
                          validator.metric_name, metric_name)
    ...
    def test_invalid_dimension_empty_key(self):
        ...
        self.assertRaises(validator.InvalidDimensionKey,
                          validator.metric_dimensions, dimensions)

    def test_invalid_dimension_empty_value(self):
        dimensions = {'A': 'B', 'B': 'C', 'D': ''}
        self.assertRaises(validator.InvalidDimensionValue,
                          validator.metric_dimensions, dimensions)
    ...
    def test_invalid_value(self):
        metric_value = "value"
        self.assertRaises(validator.InvalidValue,
                          validator.metric_value, metric_value)

    def test_valid_name_chars(self):
        for c in valid_name_chars:
            metric_name = 'test{}counter'.format(c)
            validator.metric_name(metric_name)
    ...
    def test_invalid_too_many_value_meta(self):
        ...
        self.assertRaises(validator.InvalidValueMeta,
                          validator.metric_value_meta, value_meta)
    ...
    def test_invalid_too_long_value_meta_key(self):
        key = "K"
        for i in range(0, validator.VALUE_META_NAME_MAX_LENGTH):
            key = "{}{}".format(key, "1")
        value_meta = {key: 'BBB'}
        self.assertRaises(validator.InvalidValueMeta,
                          validator.metric_value_meta, value_meta)
    ...
    def test_invalid_too_large_value_meta(self):
        value_meta_value = ""
        num_value_meta = 10
        for i in range(0, validator.VALUE_META_VALUE_MAX_LENGTH/num_value_meta):
    ...
    def test_invalid_single_metric(self):
        metric = {'name': 'test_metric_name',
                  "dimensions": {"key1": "value1",
                                 "key2": "value2"},
                  "timestamp": 1405630174123,
    ...
    def test_invalid_metrics(self):
        ...
        self.assertRaises(validator.InvalidTimeStamp,
                          validator.validate_metrics, metrics)
