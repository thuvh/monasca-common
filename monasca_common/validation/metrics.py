...
def validate(metrics):
    if isinstance(metrics, list):
        for metric in metrics:
            validate_single_metric(metric)
    else:
        validate_single_metric(metrics)


def validate_single_metric(metric):
    validate_metric_name(metric['name'])
    validate_metric_value(metric['value'])
    validate_metric_timestamp(metric['timestamp'])
    if "dimensions" in metric:
        validate_metric_dimensions(metric['dimensions'])
    if "value_meta" in metric:
        validate_metric_value_meta(metric['value_meta'])


def validate_metric_value_meta(value_meta):
...
def validate_metric_dimensions(dimensions):
...
def validate_metric_timestamp(timestamp):
